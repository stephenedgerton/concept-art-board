...
function ViewerModal({ art, categoryTags, onClose, onUpdateSuccess }: {
  ...
  const isVideo = art.type === 'animation' || art.type === 'vfx' || art.type === 'references' || (url || art.originalUrl).toLowerCase().endsWith('.mp4') || (url || art.originalUrl).toLowerCase().endsWith('.mov') || (url || art.originalUrl).toLowerCase().endsWith('.webm');
